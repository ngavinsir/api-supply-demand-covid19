...
func testUserToManyDonatorDonations(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, donationDBTypes, false, donationColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, donationDBTypes, false, donationColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.DonatorDonations().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	bFound, cFound := false, false
	for _, v := range check {
		if v.DonatorID == b.DonatorID {
			bFound = true
		}
		if v.DonatorID == c.DonatorID {
			cFound = true
		}
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadDonatorDonations(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.DonatorDonations); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.DonatorDonations = nil
	if err = a.L.LoadDonatorDonations(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.DonatorDonations); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyPasswordResetRequests(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, passwordResetRequestDBTypes, false, passwordResetRequestColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, passwordResetRequestDBTypes, false, passwordResetRequestColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.PasswordResetRequests().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadPasswordResetRequests(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.PasswordResetRequests); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.PasswordResetRequests = nil
	if err = a.L.LoadPasswordResetRequests(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.PasswordResetRequests); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyDonationApplicantRequests(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, requestDBTypes, false, requestColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, requestDBTypes, false, requestColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}

	b.DonationApplicantID = a.ID
	c.DonationApplicantID = a.ID
	...
	check, err := a.DonationApplicantRequests().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	bFound, cFound := false, false
	for _, v := range check {
		if v.DonationApplicantID == b.DonationApplicantID {
			bFound = true
		}
		if v.DonationApplicantID == c.DonationApplicantID {
			cFound = true
		}
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadDonationApplicantRequests(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.DonationApplicantRequests); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.DonationApplicantRequests = nil
	if err = a.L.LoadDonationApplicantRequests(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.DonationApplicantRequests); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpDonatorDonations(t *testing.T) {
	var err error
	...
	var a User
	var b, c, d, e Donation

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, userDBTypes, false, strmangle.SetComplement(userPrimaryKeyColumns, userColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*Donation{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, donationDBTypes, false, strmangle.SetComplement(donationPrimaryKeyColumns, donationColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*Donation{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddDonatorDonations(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.ID != first.DonatorID {
			t.Error("foreign key was wrong value", a.ID, first.DonatorID)
		}
		if a.ID != second.DonatorID {
			t.Error("foreign key was wrong value", a.ID, second.DonatorID)
		}

		if first.R.Donator != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}
		if second.R.Donator != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}

		if a.R.DonatorDonations[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.DonatorDonations[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.DonatorDonations().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testUserToManyAddOpPasswordResetRequests(t *testing.T) {
	var err error
	...
	var a User
	var b, c, d, e PasswordResetRequest

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, userDBTypes, false, strmangle.SetComplement(userPrimaryKeyColumns, userColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*PasswordResetRequest{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, passwordResetRequestDBTypes, false, strmangle.SetComplement(passwordResetRequestPrimaryKeyColumns, passwordResetRequestColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*PasswordResetRequest{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddPasswordResetRequests(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.PasswordResetRequests[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.PasswordResetRequests[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.PasswordResetRequests().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testUserToManyAddOpDonationApplicantRequests(t *testing.T) {
	var err error
	...
	foreigners := []*Request{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, requestDBTypes, false, strmangle.SetComplement(requestPrimaryKeyColumns, requestColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*Request{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddDonationApplicantRequests(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.ID != first.DonationApplicantID {
			t.Error("foreign key was wrong value", a.ID, first.DonationApplicantID)
		}
		if a.ID != second.DonationApplicantID {
			t.Error("foreign key was wrong value", a.ID, second.DonationApplicantID)
		}

		if first.R.DonationApplicant != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}
		if second.R.DonationApplicant != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}

		if a.R.DonationApplicantRequests[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.DonationApplicantRequests[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.DonationApplicantRequests().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
var (
	userDBTypes = map[string]string{`ID`: `text`, `Email`: `text`, `Password`: `text`, `Name`: `text`, `ContactPerson`: `text`, `ContactNumber`: `text`, `Role`: `text`}
	_           = bytes.MinRead
)
