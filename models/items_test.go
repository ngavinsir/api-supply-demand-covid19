...
func testItemToManyAllocationItems(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := ItemSlice{&a}
	if err = a.L.LoadAllocationItems(ctx, tx, false, (*[]*Item)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testItemToManyAddOpAllocationItems(t *testing.T) {
	var err error
