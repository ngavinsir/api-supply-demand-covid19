...
var UserWhere = struct {
	...
	ContactPerson: whereHelpernull_String{field: "\"users\".\"contact_person\""},
	ContactNumber: whereHelpernull_String{field: "\"users\".\"contact_number\""},
...
var (
	userAllColumns            = []string{"id", "email", "password", "name", "contact_person", "contact_number", "role"}
	userColumnsWithoutDefault = []string{"id", "email", "password", "name", "contact_person", "contact_number", "role"}
...
func (o *User) DonatorDonations(mods ...qm.QueryMod) donationQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"donations\".\"donator_id\"=?", o.ID),
	)
	...
	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"donations\".*"})
	}
...
func (o *User) PasswordResetRequests(mods ...qm.QueryMod) passwordResetRequestQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"password_reset_requests\".\"user_id\"=?", o.ID),
	)

	query := PasswordResetRequests(queryMods...)
	queries.SetFrom(query.Query, "\"password_reset_requests\"")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"password_reset_requests\".*"})
	}
...
func (o *User) DonationApplicantRequests(mods ...qm.QueryMod) requestQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"requests\".\"donation_applicant_id\"=?", o.ID),
	)
	...
	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"requests\".*"})
	}
...
// LoadDonatorDonations allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadDonatorDonations(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(qm.From(`donations`), qm.WhereIn(`donations.donator_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load donations")
	}

	var resultSlice []*Donation
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice donations")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on donations")
	}
	if err = results.Err(); err != nil {
	...
	if singular {
		object.R.DonatorDonations = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &donationR{}
			}
			foreign.R.Donator = object
		}
		return nil
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.DonatorID {
				local.R.DonatorDonations = append(local.R.DonatorDonations, foreign)
				if foreign.R == nil {
					foreign.R = &donationR{}
				}
				foreign.R.Donator = local
				break
			}
		}
	}
...
// LoadPasswordResetRequests allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadPasswordResetRequests(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(qm.From(`password_reset_requests`), qm.WhereIn(`password_reset_requests.user_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load password_reset_requests")
	}

	var resultSlice []*PasswordResetRequest
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice password_reset_requests")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on password_reset_requests")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for password_reset_requests")
	}
	...
	if singular {
		object.R.PasswordResetRequests = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &passwordResetRequestR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.UserID {
				local.R.PasswordResetRequests = append(local.R.PasswordResetRequests, foreign)
				if foreign.R == nil {
					foreign.R = &passwordResetRequestR{}
				}
...
// LoadDonationApplicantRequests allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadDonationApplicantRequests(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(qm.From(`requests`), qm.WhereIn(`requests.donation_applicant_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.DonationApplicantRequests = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &requestR{}
			}
			foreign.R.DonationApplicant = object
		}
		return nil
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.DonationApplicantID {
				local.R.DonationApplicantRequests = append(local.R.DonationApplicantRequests, foreign)
				if foreign.R == nil {
					foreign.R = &requestR{}
				}
				foreign.R.DonationApplicant = local
				break
			}
		}
	}
...
// AddDonatorDonations adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.DonatorDonations.
// Sets related.R.Donator appropriately.
func (o *User) AddDonatorDonations(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Donation) error {
	...
	if o.R == nil {
		o.R = &userR{
			DonatorDonations: related,
		}
	} else {
		o.R.DonatorDonations = append(o.R.DonatorDonations, related...)
	}
...
// AddPasswordResetRequests adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.PasswordResetRequests.
...
func (o *User) AddPasswordResetRequests(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*PasswordResetRequest) error {
	...
	if o.R == nil {
		o.R = &userR{
			PasswordResetRequests: related,
		}
	} else {
		o.R.PasswordResetRequests = append(o.R.PasswordResetRequests, related...)
	}

	for _, rel := range related {
		if rel.R == nil {
			rel.R = &passwordResetRequestR{
				User: o,
			}
		} else {
			rel.R.User = o
...
// AddDonationApplicantRequests adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.DonationApplicantRequests.
// Sets related.R.DonationApplicant appropriately.
func (o *User) AddDonationApplicantRequests(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Request) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.DonationApplicantID = o.ID
			if err = rel.Insert(ctx, exec, boil.Infer()); err != nil {
				return errors.Wrap(err, "failed to insert into foreign table")
			}
		} else {
	...
	if o.R == nil {
		o.R = &userR{
			DonationApplicantRequests: related,
		}
	} else {
		o.R.DonationApplicantRequests = append(o.R.DonationApplicantRequests, related...)
	}
